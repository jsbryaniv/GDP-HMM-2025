...
from architectures.blocks import conv_block_selector, FiLM3d, DyTanh3d, ConvformerDecoder3d
...
class TimeAwareUnet3d(CrossUnetModel):
    def __init__(self, 
        ...
        scale=2, feature_scale=None, adjust_context=False,
    ):
        super().__init__(
            in_channels, out_channels, n_cross_channels_list,
        ...
        # Set attributes
        self.adjust_context = adjust_context
        ...
        # Define context adjustment layers
        if adjust_context:
            self.context_adjustment_crossattn = nn.ModuleList()
            self.context_adjustment_film = nn.ModuleList()
            for depth in range(n_blocks+1):
                self.context_adjustment_crossattn.append(
                    ConvformerDecoder3d(
                        self.n_features_per_depth[depth], n_layers=1, dropout=0,
                        n_heads=max(1, min(self.n_features_per_depth[depth] // 8, 4)),
                    )
                )
                self.context_adjustment_film.append(
                    FiLM3d(self.n_features_per_depth[depth])
                )
    ...
    def _forward(self, t, x, x_pred, feats_context):
        ...
        if self.adjust_context:
            feats_context_cloned = [f.clone().detach() for f in feats_context]
            feats_context_adjusted = []
        ...
        # Adjust context
        if self.adjust_context:
            ca_crossattn = self.context_adjustment_crossattn[depth]
            ca_film = self.context_adjustment_film[depth]
            fcon = feats_context_cloned.pop()
            fcon = fcon + ca_film(ca_crossattn(fcon, x), t)
            feats_context_adjusted.append(fcon)
        ...
        # Return outputs
        if self.adjust_context:
            feats_context_adjusted = reversed(feats_context_adjusted)
            return x, feats_context_adjusted
        else:
            return x
...
class DiffUnet3d(nn.Module):
    ...
    def get_config(self):
        return {
            ...
            'eta': self.eta,
            'adjust_context': self.adjust_context,
        }
    
    def context_adjuster(self, x, feats_context):
        x = x.requires_grad_()
        feats_context = [f.requires_grad_() for f in feats_context]
        return checkpoint(self._context_adjuster, x, feats_context, use_reentrant=False)
    
    def _context_adjuster(self, x, feats_context):
        feats_pred = self.context_adjustment_encoder(x)
        feats_context = [block(fc, fx) for block, fc, fx in zip(self.context_adjustment, feats_context, feats_pred)]
    ...
    def forward(self, *context, target=None, return_loss=False):
        ...
        for t in reversed(range(1, self.n_steps)):
            ...
            # Predict x and noise
            if self.adjust_context:
                x_pred, feats_context = self.main_unet(t_step, x, x_pred, feats_context)
            else:
